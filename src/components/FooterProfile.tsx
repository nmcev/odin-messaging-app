...
export const FooterProfile: React.FC<FooterProfileProps> = ({currentUser}) => {
  ...
  return (
    ...
    <footer className="absolute bottom-0 left-0 right-0 bg-[#2b2b2b] text-white py-2 px-4 flex  items-center justify-between gap-5">
          ...
          <div className="flex items-center cursor-pointer " onClick={() => navigate(`/user/${currentUser.user.username}`)}>
              ...
              <div className="ml-4">
                <h2 className="text-xl font-bold">{currentUser.user.username}</h2>
              </div>
          </div>
